...
realized that we were calling \texttt{malloc} left and right. These leaks can
certainly become a serious issue, especially in a long running program.
...
While outputting object files and linking against C++ code would certainly be
useful, the absence of this feature allows us to add some optimizations that
aren't possible in C++ (though we don't currently implement them). We are in
a ``closed world'' when compiling C0 because we know exactly what code will be
running at compile time. Given that we have all the code that makes up the
program, we could optimize out many of the vtable look ups for most programs.
If the String class is never extended and the \texttt{toUpperCase()} function is
called on a string, then we know at compile time which function should be called
because there is only one \texttt{toUpperCase()} implementation defined. This
optimization would eliminate both memory accesses for method invocation, which
could have a major impact on performance. While this optimization would
certainly result in better code and bring the object oriented performance to the
level of regular C0, we did not have time to implement it.
