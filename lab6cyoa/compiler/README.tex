...
When a \texttt{Bar} is cast to a \texttt{Foo}, the same instructions
can be used to call \texttt{Bar}'s \texttt{fun2} as opposed to \texttt{Foo}'s.
We had two options of where to put the vtables. They could either be included in
every object, or they could be located in the data section of the executable.
Placing a vtable copy in every object would eliminate one memory access per
method invocation, but it could dramatically increase the size of objects.
We decided to just have a single copy of each vtable, and have each object store
a pointer to its corresponding vtable.
