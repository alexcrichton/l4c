...
struct Spiller {
    ...
    deltas: HashMap<NodeId, Vec<Vec<(Temp, Option<uint>)>>>,
    ...
    renamings: HashMap<(NodeId, NodeId), Box<HashMap<Temp, Vec<Temp>>>>,
    ...
    connected: HashMap<(NodeId, NodeId), Vec<Box<Instruction>>>,
}
...
fn sort(set: &TempSet, s: &NextUse) -> Vec<Temp> {
    let mut v = set.iter().map(|&t| t).collect::<Vec<Temp>>();
...
impl Spiller {
                    ...
                    for (&pred, &their_name) in renamings.iter() {
                        match self.renamings.find_mut(&(pred, n)) {
                            Some(m) => {
                                ...
                                m.insert(their_name, vec![my_name]);
                                continue
                            }
                            None => ()
                        }
                        let mut m = box HashMap::new();
                        m.insert(their_name, vec![my_name]);
    ...
    fn build_next_use(&mut self, f: &Function, n: NodeId) -> bool {
        ...
        for (i, ins) in indexes.zip(block.iter().rev()) {
    ...
    fn spill(&mut self, f: &mut Function, n: NodeId) {
        ...
        fn apply_delta(delta: &Vec<(Temp, Option<uint>)>,
                       next_use: &mut HashMap<Temp, uint>) {
            ...
            match ins {
                ...
                box Phi(tmp, map) => {
                    if regs.contains(&tmp) {
                        block.push(box Phi(tmp, map));
                    } else if next_use.contains_key(&tmp) {
                        block.push(box MemPhi(tmp, map));
                    }
                ...
                box PCopy(copies) => {
                    ...
                    block.push(box PCopy(newcopies));
                    apply_delta(delta, &mut next_use);
                }
                ...
                ins => {
                    ...
                    let extra = match ins {
                        box BinaryOp(op, _, _, _) if op.constrained() => 1,
                        box Call(..) => arch::caller_regs,
                        _ => 0
                    };
    ...
    fn connect_edge(&mut self, pred: NodeId, succ: NodeId) {
        ...
        for &tmp in succ_spill.iter() {
            ...
            if !pred_spill_exit.contains(&theirs) &&
                pred_regs_exit.contains(&theirs) {
                    append.push(box Spill(theirs, theirs));
                }
        }
        ...
        for &tmp in succ_regs.iter() {
            ...
            if !pred_regs_exit.contains(&theirs) {
                append.push(box Reload(theirs, theirs));
            }
        }
