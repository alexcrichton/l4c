...
struct
  ...
  fun make_graph ([], []) _ = ()
    | make_graph (set::S, i::L) graph = let
        val temp_set = OperandSet.foldr (fn (AS.TEMP t, s) => TempSet.add (s, t)
                                          | (_, s) => s)
                                        TempSet.empty set
        (* handle pre-coloring *)
        fun precolor (AS.REG reg, set) = let
              val t1 = Temp.new()
              val set' = TempSet.add (set, t1)
              val () = G.setValue graph (t1, {color=(AS.reg_num reg), weight=0})
            in
              set'
            end
          | precolor (_, s) = s

        val temp_set' = OperandSet.foldl precolor temp_set set
        val () = G.addClique graph temp_set'
      in
