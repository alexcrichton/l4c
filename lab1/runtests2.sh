...
# Signal handler - makes sure the right signal was triggered
function signal() {
  local signum=$(($?-128))
  local expected=$1
  [ $signum -eq $expected ] && pass || fail "Got signal $signum, expected $expected"
}
    ...
    exception)
      if [ $ret -eq 0 ]; then
        ...
        if [ $ret -eq 0 ]; then
          ...
          program=${test/%.l1/}
          trap "signal $expected" 4 6 8 9 11 13 15
          set +e
          output=`$program`
          set -e
